...
function Y() {
    ...
    jQuery('#homeLoneAmount').val(jQuery("#loanamount").val());
    jQuery('#homeInterestRate').val(jQuery("#loaninterest").val());
    jQuery('#homeLoanTenure').val(jQuery("#loanterm").val()+ ' '+ document.querySelector('input[name="loantenure"]:checked').value);
    jQuery('#homeLoanEmi').val('â‚¹ '+ Globalize.format(Math.round(h), "n", "en-IN"));
    jQuery('#homeTotalInterestPayable').val('â‚¹ '+ Globalize.format(Math.round(h * g - c), "n", "en-IN"));
    jQuery('#homeLoanTotalPayment').val('â‚¹ '+ Globalize.format(Math.round(h * g), "n", "en-IN"));
    if(jQuery("#loanamount").val()){
        var projData = "<div style='width:100%; display:block; position:relative;'>";
        $( ".hotels-list-w.recommended-projects #recomProject .hotels-item" ).each(function() {
            if(parseInt($(this).attr('data-loan')) > parseInt(jQuery("#loanamount").val().replace(/,/g , '')) )
            {
                $(this).addClass('hideBoxItem');
            }
            else{
                projData += "<div style='width:32%; display:inline-block; float:left;padding: 0;border: 2px solid #333;margin: 5px;'><h3 style='text-align:center'>"+$(this).find('.product-details h6').text()+"</h3><img src='https://sunbuilders.in/"+$(this).find('img').attr('src')+"' style='width:100%; display:block'><div style='width:100%; display:block;padding: 15px;'><p style='display:block; margin:0 0 10px 0; color:#333'>Possession Status: <b>"+$(this).find('.product-details .posseStatus').text()+"</b></p><p style='display:block; margin:0 0 10px 0; color:#333'>Typology: <b>"+$(this).find('.product-details .typology').text()+"</b></p><p style='display:block; margin:0 0 10px 0; color:#333'>Location: <b>"+$(this).find('.product-details .location').text()+"</b></p><p style='display:block; margin:0 0 10px 0; color:#333'>Price: <b>"+$(this).find('.product-details .price').text()+"</b></p><a href='https://sunbuilders.in/"+$(this).find('a').attr('href')+"' style='display:block; width:fit-content;'>View Details</a></div></div>";
                $(this).removeClass('hideBoxItem');
            }
        });
        projData += "</div>";
        $('#projectListData').val(projData);
    }
    ...
    new Highcharts.Chart({
        ...
        yAxis: [{
            ...
            labels: {
                ...
                formatter: function() {
                    ...
                    return "â‚¹ " + Globalize.format(this.value, "n", "en-IN")
        ...
        tooltip: {
            ...
            formatter: function() {
                ...
                return "Balance" == this.series.name ? "<b>Year: " + this.x + "</b><br/>" + this.series.name + " : â‚¹ " + Globalize.format(this.y, "n", "en-IN") + "<br/>Loan Paid To Date : " + Globalize.format((c - this.y) / c * 100, "n2", "en-IN") + "%" : "<b>Year : " + this.x + "</b><br/>" + this.series.name + " : â‚¹ " + Globalize.format(this.y, "n", "en-IN") + "<br/>Total Payment : â‚¹ " + Globalize.format(this.point.stackTotal, "n", "en-IN")
...
function ba() {
    ...
    for (var a = 0, e = 0, s = w.length; s > e; e++) {
        ...
        n += '<tr class="yearlypaymentdetails"><td id="year' + t + '" class="col-xs-2 col-md-1 paymentyear toggle">' + t + '</td><td class="col-xs-3 col-sm-2 currency">â‚¹ ' + Globalize.format(x[t], "n", "en-IN") + '</td><td class="col-xs-3 col-sm-2 currency">â‚¹ ' + Globalize.format(y[t], "n", "en-IN") + '</td><td class="col-sm-3 hidden-xs currency">â‚¹ ' + Globalize.format(x[t] + y[t], "n", "en-IN") + '</td><td class="col-xs-4 col-sm-3 currency">â‚¹ ' + Globalize.format(z[t], "n", "en-IN") + '</td><td class="col-md-1 hidden-xs hidden-sm paidtodateyear">' + Globalize.format(A[t], "n2", "en-IN") + "%</td></tr>", n += '<tr id="monthyear' + t + '" class="monthlypaymentdetails"><td class="row monthyearwrapper" colspan="6"><div class="monthlypaymentcontainer"><table>';

        for (var l = B.length; l > a && B[a].getFullYear() == t;) n += '<tr><td class="col-xs-2 col-md-1 paymentmonthyear">' + I[B[a].getMonth()] + '</td><td class="col-xs-3 col-sm-2 currency">â‚¹ ' + Globalize.format(D[a], "n", "en-IN") + '</td><td class="col-xs-3 col-sm-2 currency">â‚¹ ' + Globalize.format(E[a], "n", "en-IN") + '</td><td class="col-sm-3 hidden-xs currency">â‚¹ ' + Globalize.format(D[a] + E[a], "n", "en-IN") + '</td><td class="col-xs-4 col-sm-3 currency">â‚¹ ' + Globalize.format(F[a], "n", "en-IN") + '</td><td class="col-md-1 hidden-xs hidden-sm paidtodatemonthyear">' + Globalize.format(G[a++], "n2", "en-IN") + "%</td></tr>";

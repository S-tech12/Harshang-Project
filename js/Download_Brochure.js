...
{	
	...
	if (!c_pattern.test(c_mobile)) {
	 ...
	 if ($('#whatsapp_agree').is(':checked')) {
                    ...
                    alert('please agree for notification updates over WhatsApp.');
                    return false;
                }   
	...
	$.ajax({
			   ...
			   {
				   jQuery('.popup form#Download_Brochure + .fade-out').css('visibility','visible');
				   jQuery("#Download_Brochure").hide();
                   jQuery("#Download_Brochure").after(data);
			   ...
			   error: function(data)
			   {
				   jQuery('.popup form#Download_Brochure + .fade-out').css('visibility','visible');
				   jQuery("#Download_Brochure").hide();
                   jQuery("#Download_Brochure").after("<div class='alert alert-danger'>Sorry! Some Technical issue occured. Please try again after sometime.</div>"); // show response from the php script.
				    }
			   
			 });

...
{	
	...
	if(email.value=='')
	{
		document.querySelector('#Inquiry #error_data').innerHTML = '* Please Enter Email ID.';
	...
	if(c_reg.test(c_address) == false) {
		document.querySelector('#Inquiry #error_data').innerHTML = '* Invalid Email ID: ' + email.value;
		email.style.borderColor="red";
		email.value = '';
		email.focus();
		return false;
	...
	if(contact_no.value=='')
	{
		document.querySelector('#Inquiry #error_data').innerHTML = '* Please Enter Contact No.';
	...
	if (!c_pattern.test(c_mobile)) {
		document.querySelector('#Inquiry #error_data').innerHTML = '* Invalid Contact No.: ' + contact_no.value;
		contact_no.style.borderColor="red";
		contact_no.value = '';
        contact_no.focus();
		return false;
	...
	var time_of_appointment=document.querySelector('#Inquiry #time_of_appointment');	
	if(!document.querySelector('input[name="time_of_appointment"]:checked'))
	{
		document.querySelector('#Inquiry #error_data').innerHTML = '* Please Enter your Appointment Time.';
		// time_of_appointment.style.borderColor="red";
		// time_of_appointment.focus();
		return false;
	}
	// else{time_of_appointment.style.borderColor=""}	
	...
	if(valid_contact()==true){document.querySelector('#Inquiry #form_process').style.visibility="visible";jQuery(this).find('[type="submit"]').prop('disabled', true);//.fadeOut('slow');
		...
		$.ajax({
			   ...
			   success: function(data)
			   {
				   ...
				   jQuery("#Inquiry").html(data); // show response from the php script.
			   },
			   error: function(data)
			   {
				   jQuery("#Inquiry").empty();
